...
export default class Physics{
    ...
    physicStep(dt: number, inputDir :Vector, collisions: BoundingBox[]){//Euler integration
        ...
        if(this.vel.length < 0.01 ){//TO DO evaluate the value
            this.vel = new Vector(0,0)
        }        

        let pos: Vector = this.position.clone
