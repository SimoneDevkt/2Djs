...
export default class Rendering {
    ...
    commandCallBack: (canvas: HTMLCanvasElement) => void
    declare half: Vector
    camera: Camera
    constructor(id: string, camera: Camera, commandCallBack: (canvas: HTMLCanvasElement) => void) {
    ...
    private drawEntity(e: EntityRender) {
        //draw a tile in tileset
        const { n, height, width, canvasImageSource, columns, position, globalAlpha } = e
        if (n > -1) {
